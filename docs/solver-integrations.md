...
Workers must implement `LaunchRequest` through the manifest-specific typed command builder and return `RunRecord` through `SolverGateway`. A case is an opaque relative identifier (no separators, drive roots, absolute paths, or traversal) resolved beneath the approved job root. They cannot launch if the corresponding capability is unavailable. Solver probes are bound to the immutable registry's executable/version probe; no caller-supplied command or arbitrary `string[]` reaches the runner. Checkpoints are explicit lineage references, never automatic recovery claims. Native result publication is disabled until a completed run receipt, approved artifact root, parser receipt, exact 64-hex SHA-256, solver version, and input/checkpoint lineage are all verified.
