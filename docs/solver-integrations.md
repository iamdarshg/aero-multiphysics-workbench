...
Workers must implement `LaunchRequest` and return `RunRecord` through `SolverGateway`. They cannot launch if the corresponding capability is unavailable. Solver executables come only from the manifest allowlist and arguments are validated for shell metacharacters before spawning. Checkpoints are explicit lineage references, never automatic recovery claims. Results need an accompanying provenance event and must state whether they are native, analytical, benchmark, or surrogate output.
