...
import { getDemoProfile, getDockPanel, getViewModeStatus, type DemoId, type DockTab, type ViewMode, updateCouplingStrength } from '../workbench-state';
...
export default function WorkbenchPage() {
  ...
  return <main className="workbench">
    ...
    <section className="coupling-bar">
      <div><span className="muted-label">COMPUTE TARGET</span><strong>Local · native solver gate closed</strong></div>
    ...
    <div className="workspace-grid">
      <aside className="left-rail">
        ...
        <div className="panel-heading compact"><div><span className="muted-label">VARIANTS</span><strong>Design branches</strong></div><button className="icon-button" title="Create a future variant"><Icon name="add" /></button></div>
      ...
      <aside id="workbench-inspector" className={`right-rail ${inspectorOpen ? 'mobile-open' : ''}`}>
        <div className="panel-heading"><div><span className="muted-label">INSPECTOR</span><strong>{activeNode}</strong></div><div className="inspector-heading-actions"><button className="close-inspector" onClick={() => setInspectorOpen(false)}>Close</button><button className="icon-button" title="Inspector options"><Icon name="more" /></button></div></div>
        ...
        <div className="inspector-section"><span className="muted-label">QUALITY GATES</span><div className="quality-row"><i className="good" />Energy balance <b>0.8%</b></div><div className="quality-row"><i className="watch" />Resonance margin <b>1.21×</b></div><div className="quality-row"><i className="blocked" />Native execution <b>Unavailable</b></div></div>
