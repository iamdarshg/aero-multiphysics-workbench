...
def main() -> int:
    out: dict = {"benchmark": "electrical-machine-drive-battery-energy-closure",
                 "status": "BLOCKED"}
    try:
    ...
    try:
        ...
        for speed, torque in ((20000.0, 0.0), (20000.0, 0.01), (12000.0, 0.0)):
            try:
                ...
                cases.append({"speedRpm": speed, "loadTorqueNm": torque,
            ...
            consumed = (mechanical + machine_loss + drive_loss
                        if None not in (mechanical, machine_loss, drive_loss) else None)
            power = None
            heat = None
            if None not in (electrical, mechanical, machine_loss):
                power = assess_closure("energy", {"energy": mechanical + machine_loss},
                                       {"energy": electrical})
            if None not in (machine_loss, thermal_machine, drive_loss, thermal_drive):
                heat = assess_closure("heat-balance",
                                      {"heat-flow": thermal_machine + thermal_drive},
                                      {"heat-flow": machine_loss + drive_loss},
                                      required=("heat-flow",))
            cases.append({
                "speedRpm": speed, "loadTorqueNm": torque,
                "engine": result.engine, "converged": result.converged,
                "iterations": result.iterations, "residualNorm": result.residual_norm,
                "closureResidual": result.closure_residual,
                "electricalPowerW": electrical, "mechanicalPowerW": mechanical,
                "machineLossW": machine_loss, "driveLossW": drive_loss,
                "deliveredPowerW": delivered, "consumedPowerW": consumed,
                "couplingClosureResidualW": (
                    None if None in (delivered, consumed) else delivered - consumed),
                "thermalMachineLossW": thermal_machine, "thermalDriveLossW": thermal_drive,
