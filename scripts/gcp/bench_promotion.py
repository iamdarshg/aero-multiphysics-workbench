...
def main() -> int:
    ...
    from aeroworkbench_convergence import (
        PromotionParticipantEvidence, QuantityOfInterest, RefinementLevel,
        StudyRun, assess_promotion, run_mesh_independence,
    )
    ...
    def of_executor(level: RefinementLevel) -> StudyRun:
        ...
        return StudyRun(level=level.name, run_id=f"of-{level.name}",
    ...
    def el_executor(level: RefinementLevel) -> StudyRun:
        ...
        return StudyRun(level=level.name, run_id=f"el-{level.name}",
    ...
    try:
        ...
        if len(of_lv) >= 2:
            report = run_mesh_independence(
                tuple(RefinementLevel(name, 1.0 / max(cells, 1) ** 0.5,
                                      (("element_count", float(cells)),))
    ...
    try:
        if len(el.get("levels", [])) >= 2:
            report = run_mesh_independence(
                tuple(RefinementLevel(f"lc{x['lc']}", x["lc"],
                                      (("lc", x["lc"]),)) for x in el["levels"]),
    ...
    def participant(pid: str, available: bool, *, converged=True, closure=True,
                    coupling=True, mesh=True, timestep=True, deferred=False):
        return PromotionParticipantEvidence(
            participant_id=pid, required=True, available=available, deferred=deferred,
            converged=converged, validity_ok=True, closure_passed=closure,
            field_coupling_passed=coupling, mesh_independence_passed=mesh,
            timestep_independence_passed=timestep)

    participants = ("cad-interchange", "domain-mesh", "incompressible-steady-flow",
                    "structural-static", "thermal-conduction", "rotor-campbell",
                    "rotating-electrical-machine", "coupled-interface-validation")
    ...
    ok = assess_promotion("candidate-all-receipts", full_evidence(),
                          require_mesh_independence=True, require_timestep_independence=True)
    out["positiveControl"] = ok.as_dict()

    # 2) remove one required receipt -> must refuse
    missing = full_evidence(**{"thermal-conduction": participant(
        "thermal-conduction", False, deferred=True)})
    no_thermal = assess_promotion("candidate-missing-thermal", missing,
    ...
    bad_mesh = full_evidence(**{"incompressible-steady-flow": participant(
        "incompressible-steady-flow", True, mesh=False)})
    bad = assess_promotion("candidate-bad-mesh-independence", bad_mesh,
                           require_mesh_independence=True,
                           require_timestep_independence=True)
    ...
    out["honestSet"] = assess_promotion("candidate-real-receipts", real,
                                        require_mesh_independence=True,
                                        require_timestep_independence=True).as_dict()
    ...
    print("WROTE issue41_gate.json status=", out["status"],
          "positive=", ok.validated_final, "refusesInvalid=", out["refusesInvalid"])
    return 0
