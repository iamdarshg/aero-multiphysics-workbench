...
PARTICIPANT_MANIFESTS: tuple[ParticipantManifest, ...] = (
    ...
    _manifest(
        ...
        "coupled",
        "Native preCICE implicit field coupling between two generated-interface participants.",
        (
            _scalar("coupling_dt_s", "s", "in"),
            _scalar("max_iterations", "dimensionless", "in", "int"),
            _scalar("tolerance", "dimensionless", "in"),
            _scalar("n_interface_points", "dimensionless", "in", "int"),
        ),
        (
            _scalar("interface_residual", "dimensionless", "out"),
            _scalar("conservation_error", "dimensionless", "out"),
            _scalar("coupling_iterations", "dimensionless", "out", "int"),
            _scalar("checkpoints", "dimensionless", "out", "int"),
            _scalar("rollbacks", "dimensionless", "out", "int"),
            _scalar("coupled_converged", "dimensionless", "out", "bool"),
        ),
        _executable("precice", ("python",), run_script="run_precice.py"),
        "precice.validate:prepare_native_window",
        "precice.validate:parse_native_window_result",
        "precice.validate:validate_native_window_result",
        ("precice-config.xml", "case.json", "result.json", "solver.log"),
        geometry_roles=(),
        mesh_roles=(),
        semantic_requirements=("coupling_interface",),
        coupling_direction="two-way",
        convergence_measures=("interface_residual", "conservation"),
        fidelity_levels=("implicit-iqn",),
        checkpoint_policy=CheckpointMode.SOLVER_WINDOW,
        benchmark_ref="gen11:native-coupled-window",
    ),
    _manifest(
        "domain-mesh",
        "mesh",
